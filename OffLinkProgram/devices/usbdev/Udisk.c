...
__attribute__ ((aligned(4))) uint8_t  UDisk_Down_Buffer[4096];
...
BULK_ONLY_CMD mBOC;
uint8_t   *pEndp2_Buf;
static struct udisk_function *fun;
...
{
    /* UDISK */
    0x02,                                                /* Peripheral Device Type:UDISK = 0x00 */
    0x80,                                                /* Removable */
    0x02,                                                /* ISO/ECMA */
    0x02,
...
void UDISK_Rd_Format_Capacity(uint8_t *data)
{
    uint8_t len = 0;
    data[len++] = 0x00;
    data[len++] = 0x00;
    data[len++] = 0x00;
    data[len++] = 0x08;
    data[len++] = (fun->Udisk_Sector_size >> 24) & 0xFF;
    data[len++] = (fun->Udisk_Sector_size >> 16) & 0xFF;
    data[len++] = (fun->Udisk_Sector_size >> 8) & 0xFF;
    data[len++] = (fun->Udisk_Sector_size) & 0xFF;
}

/******************************************************************************/
/* capacity information */
void UDISK_Rd_Capacity(uint8_t *data)
{
    uint8_t len = 0;
    data[len++] = ((fun->Udisk_Sector_size - 1) >> 24) & 0xFF;
    data[len++] = ((fun->Udisk_Sector_size - 1) >> 16) & 0xFF;
    data[len++] = ((fun->Udisk_Sector_size - 1) >> 8) & 0xFF;
    data[len++] = (fun->Udisk_Sector_size - 1) & 0xFF;
    data[len++] = (fun->Udisk_Sector_size >> 24) & 0xFF;
    data[len++] = (fun->Udisk_Sector_size >> 16) & 0xFF;
    data[len++] = (fun->Udisk_Sector_size >> 8) & 0xFF;
    data[len++] = (fun->Udisk_Sector_size) & 0xFF;
...
void UDISK_Mode_Sense_1A(uint8_t *data)
{
    uint8_t len = 0;
    data[len++] = 0xB0;
    data[len++] = 0x00;
...
void UDISK_Mode_Senese_5A(uint8_t *data)
{
    uint8_t len = 0;
    data[len++] = 0x00;
    data[len++] = 0x0E;
    data[len++] = 0x00;
    data[len++] = 0x00;                                              /* 0x00:write-unprotected,0x80:write-protected */
    data[len++] = 0x00;
    data[len++] = 0x00;
    data[len++] = 0x00;
    data[len++] = 0x08;
...
void CMD_RD_WR_Deal_Pre( void )
{
    ...
    UDISK_Transfer_DataLen = UDISK_Transfer_DataLen * fun->Udisk_Sector_size;
      ...
      &&( mBOC.mCBW.mCBW_Sig[ 2 ] == 'B' ) && ( mBOC.mCBW.mCBW_Sig[ 3 ] == 'C' ) )
    {
        ...
        switch( mBOC.mCBW.mCBW_CB_Buf[ 0 ] )
        {
            case  CMD_U_INQUIRY:                                                                    
                ...
                UDISK_Inquity_Tab[ 32 ] =  (uint8_t)( fun->Flash_ID >> 24 );
                UDISK_Inquity_Tab[ 33 ] =  (uint8_t)( fun->Flash_ID >> 16 );
                UDISK_Inquity_Tab[ 34 ] =  (uint8_t)( fun->Flash_ID >> 8 );
                UDISK_Inquity_Tab[ 35 ] =  (uint8_t)( fun->Flash_ID );
            ...
            case  CMD_U_READ_FORMAT_CAPACITY:                                     
                ...
                {    
                    ...
                    UDISK_Rd_Format_Capacity(mBOC.buf);
                    pEndp2_Buf = mBOC.buf;   
                    UDISK_CMD_Deal_Status( 0x00, 0x00, 0x00 );
                }
                else
...
{                                            
    ...
    fun->Udisk_Usb_Send(pEndp2_Buf, len );
}
...
void UDISK_Up_CSW( void )
{
    ...
    /* Load the data into the upload buffer and start the upload */
    fun->Udisk_Usb_Send((uint8_t *)mBOC.buf, 0x0D );
}
...
{    
    ...
    fun->Udisk_Read_Data(UDISK_Cur_Sec_Lba * fun->Udisk_Sector_size + UDISK_Pack_Size * UDISK_Sec_Pack_Count,UDisk_Pack_Buffer, UDISK_Pack_Size);
    /* USB upload this package data */

    fun->Udisk_Usb_Send(UDisk_Pack_Buffer, UDISK_Pack_Size );

    /* Determine whether the current sector data is read and uploaded */
    UDISK_Sec_Pack_Count++;
    UDISK_Transfer_DataLen -= UDISK_Pack_Size;

    if( UDISK_Sec_Pack_Count == ( fun->Udisk_Sector_size / UDISK_Pack_Size ) )
    {
        UDISK_Sec_Pack_Count = 0x00;
        UDISK_Cur_Sec_Lba++;
    }
...
void UDISK_Down_OnePack( uint8_t *pbuf, uint16_t packlen )
{
    ...
    static uint32_t sec_start_addr;
    if( UDISK_Sec_Pack_Count == 0x00 )
    {
        address = (uint32_t)UDISK_Cur_Sec_Lba * fun->Udisk_Sector_size;
        sec_start_addr = ( address / fun->Udisk_Sector_size ) * fun->Udisk_Sector_size;
        fun->Udisk_Erase_Sector(sec_start_addr);
    }
    ...
    if( UDISK_Sec_Pack_Count == ( fun->Udisk_Sector_size / UDISK_Pack_Size ) )
    {
        address = (uint32_t)UDISK_Cur_Sec_Lba * fun->Udisk_Sector_size;
        sec_start_addr = ( address / fun->Udisk_Sector_size ) * fun->Udisk_Sector_size;
        fun->Udisk_Write_Data(sec_start_addr,UDisk_Down_Buffer,  fun->Udisk_Sector_size );
        if( UDISK_Transfer_DataLen == 0x00 )
        {
            Udisk_Transfer_Status &= ~DEF_UDISK_BLUCK_DOWN_FLAG;
            UDISK_Up_CSW( );
